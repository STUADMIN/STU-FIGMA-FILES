...
export default function LoginPage() {
  ...
  const handlePasswordSignIn = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error && (error as any).code === "mfa_required") {
        toast.info("Two-factor required. Enter your code to continue.");
      ...
      if (data.session) {
        ...
        router.replace(hasTotp ? "/auth/verify" : "/");
        return;
      }
    ...
    <div className="min-h-dvh bg-[#0D2352] text-white flex items-center justify-center p-6">
      <div className="w-full max-w-md rounded-xl bg-[#0f1838]/60 p-8 shadow-xl border border-white/5">
        ...
        <form onSubmit={handlePasswordSignIn} className="space-y-4">
          <div>
            <label className="mb-1 block text-sm text-white/90">Username</label>
            <input
              type="email"
              value={email}
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full rounded-md bg-white/5 px-3 py-2 pr-10 text-white outline-none ring-1 ring-white/10 focus:ring-2 focus:ring-white/20"
              ...
              <button
                ...
                className="absolute inset-y-0 right-2 my-auto grid h-7 w-7 place-items-center rounded-md text-white/80 hover:text-white"
              >
                {showPassword ? (
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="h-5 w-5"><path d="M3.53 2.47a.75.75 0 1 0-1.06 1.06l18 18a.75.75 0 1 0 1.06-1.06l-3.112-3.112A12.34 12.34 0 0 0 21.75 12S18.75 5.25 12 5.25a9.6 9.6 0 0 0-4.05.87L3.53 2.47zM6.32 7.26A10.79 10.79 0 0 1 12 6.75c5.215 0 7.924 3.984 8.674 5.25a11.85 11.85 0 0 1-2.296 2.79l-2.045-2.045a4.5 4.5 0 0 1-6.03-6.03L6.32 7.26zM9.53 10.47a2.25 2.25 0 0 1 3 3l-3-3zM4.572 9.07a.75.75 0 0 0-1.144.974C4.43 11.205 7.24 15.75 12 15.75c.835 0 1.62-.114 2.35-.32l-1.59-1.59a4.5 4.5 0 0 1-5.6-5.6L4.572 9.07z"/></svg>
                ) : (
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="h-5 w-5"><path d="M12 5.25C5.25 5.25 2.25 12 2.25 12s3 6.75 9.75 6.75 9.75-6.75 9.75-6.75-3-6.75-9.75-6.75zm0 10.5A3.75 3.75 0 1 1 12 8.25a3.75 3.75 0 0 1 0 7.5z"/></svg>
                )}
              </button>
            </div>
          </div>
          <button type="submit" disabled={loading} className="w-full rounded-md bg-[#4c7cf0] hover:bg-[#3b6be0] px-4 py-2 font-medium text-white transition disabled:opacity-60">
            {loading ? "Signing in…" : "Login"}
          </button>
        </form>

        <div className="mt-3 text-center text-sm">
          <Link href="/reset-password" className="text-white/80 hover:text-white underline">Reset password</Link>
        </div>

        <div className="mt-6 text-center text-sm text-white/80">
          Don&apos;t have an account? <Link href="/register" className="underline hover:text-white">Register here</Link>
        </div>
      </div>
    </div>
  );
