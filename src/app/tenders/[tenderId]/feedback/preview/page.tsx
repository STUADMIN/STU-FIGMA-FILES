...
type DecodeResult =
	| { value: FeedbackPreviewData; raw: string }
	| { value: null; raw: string | null; reason: string };

function decodeFeedback(raw: string | null): DecodeResult {
	if (!raw) {
		return { value: null, raw, reason: "no query param" };
	}

	// We’ll try multiple strategies:
	const candidates: string[] = [raw];

	// If it was URI encoded, add that candidate too
	try {
	...
	// Try: JSON directly, then base64->JSON for each candidate
	for (const candidate of candidates) {
		...
		// 2) base64 -> JSON
		try {
			const jsonStr = typeof atob !== "undefined" ? atob(candidate) : candidate;
	...
	return { value: null, raw, reason: "unable to decode payload" };
}
...
export default function FeedbackPreviewPage() {
	...
	const decodeResult = useMemo(
		() => decodeFeedback(searchParams.get("data")),
		[searchParams]
	);

	const feedback = decodeResult.value;
			...
			<main className="min-h-dvh bg-[#f5f5f5]">
				<div className="mx-auto max-w-4xl px-4 py-12">
					<div className="rounded-2xl bg-white p-8 shadow-sm">
						...
						{/* TEMP: small debug so we can see what’s coming through */}
						<div className="mt-4 rounded-md bg-[#f3f4f6] px-3 py-2 text-xs text-[#4b5563]">
							<div className="font-semibold mb-1">Debug info (safe):</div>
							<div className="mb-1">
								<span className="font-semibold">Raw &quot;data&quot; param:</span>{" "}
								{decodeResult.raw ? String(decodeResult.raw).slice(0, 120) + "…" : "null"}
							</div>
							<div>
								<span className="font-semibold">Decode reason:</span>{" "}
								{"reason" in decodeResult ? decodeResult.reason : "decoded"}
							</div>
						</div>
					</div>
				</div>
