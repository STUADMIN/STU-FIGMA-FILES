...
import { StuIcon } from "../icons/StuIcon";
...
export default function AppShell({ children, userDisplayName, fullWidth = false }: AppShellProps) {
  const pathname = usePathname();
  const isActive = (href: string) => (pathname === href || pathname.startsWith(href)) && href !== "#";
  const contentWrapperClasses = fullWidth ? "w-full px-0 pb-6" : "mx-auto w-full max-w-[1680px] px-6 py-5";
  const navWrapperClasses = "mx-auto flex h-16 w-full max-w-[1680px] items-center justify-between px-7";

  return (
    <main className="min-h-dvh bg-white text-gray-900">
      <div className="flex min-h-dvh">
        <aside className="relative hidden w-60 shrink-0 bg-brand-navy text-white md:flex md:flex-col pt-[160px] pb-12">
          <div className="absolute left-[30px] top-[40px] h-[127px] w-[162px]">
          ...
          <nav className="mt-2 grid gap-1 px-2">
            ...
            <NavLink href="/tenders" active={isActive("/tenders")} label="Tenders" icon="tenders" />
            <NavLink href="#" active={false} label="Projects" icon="projects" />
            <NavLink href="#" active={false} label="Health &amp; Safety" icon="health-safety" />
            <NavLink href="/equipment" active={isActive("/equipment")} label="Equipment" icon="equipment" />
            <NavLink href="#" active={false} label="Vehicles" icon="vehicles" />
            <NavLink href="#" active={false} label="Emissions" icon="emissions" />
            <NavLink href="#" active={false} label="Accounts" icon="accounts" />
            <NavLink href="#" active={false} label="HR" icon="hr" />
          </nav>
          <div className="mt-16 px-4 text-sm opacity-80">
            <Link
              href="/help-support"
              className={`flex items-center gap-3 rounded-md px-3 py-2 transition-colors hover:bg-white/10 ${isActive("/help-support") ? "bg-white/10 font-medium" : "opacity-90"}`}
            >
            ...
            <div className={navWrapperClasses}>
              <div className="relative w-[320px]">
                <input
                  type="text"
                  placeholder="Search..."
                  className="h-12 w-full rounded-[20px] border border-[#BBC9CE] bg-white pl-10 pr-4 text-sm text-gray-700 placeholder-gray-400 outline-none"
                  aria-label="Search"
                />
                <svg
                  width="24"
              ...
              <div className="flex h-12 items-center gap-5 text-brand-navy">
                <StuIcon name="bell" size={20} className="text-brand-navy" aria-label="Notifications" />
                <StuIcon name="person" size={24} className="text-brand-navy" aria-label="Profile" />
                <StuIcon name="settings" size={20} className="text-brand-navy" aria-label="Settings" />
                <UserPill initialName={userDisplayName ?? ""} />
              </div>
            </div>
          </div>
...
function NavLink({
  ...
  return (
    <Link
      href={href}
      className={`flex items-center gap-3 rounded-md px-4 py-3 text-sm transition-colors hover:bg-white/10 ${active ? "bg-white/10 font-medium" : "opacity-90"}`}
    >
      <StuIcon name={icon} size={18} className={active ? "text-brand-menu-active" : "text-white"} aria-label={label} />
      {label}
    </Link>
  );
}
